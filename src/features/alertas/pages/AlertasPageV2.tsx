...
import { 
  AlertTriangle, Shield, TrendingUp, Clock, Users, CheckCircle, 
  History, Bell, BellOff, Filter, RefreshCw, User, MessageSquare,
  XCircle, CheckCircle2, AlertCircle, Zap
} from 'lucide-react';
...
const AlertRow: React.FC<{
  ...
  onView: (alerta: Alerta) => void;
  index: number;
}> = ({ alerta, onAtender, onAsignar, onView, index }) => {
...
const AlertasPageV2: React.FC = () => {
  ...
  const handleAtenderAlerta = async (alerta: Alerta) => {
    await atenderAlerta(alerta.id);
  };

  const handleAsignarAlerta = async (alerta: Alerta) => {
    // Aquí podrías abrir un modal para seleccionar usuario
    const usuario = prompt('Asignar a usuario:');
    if (usuario) {
      await asignarAlerta(alerta.id, usuario);
    }
  };

  const handleViewAlerta = (alerta: Alerta) => {
                        ...
                        <AnimatePresence mode="popLayout">
                          {filteredAlertas.map((alerta, index) => (
                            <AlertRow
                              ...
                              onAsignar={handleAsignarAlerta}
                              onView={handleViewAlerta}
                              index={index}
                            />
                          ))}
                        </AnimatePresence>
