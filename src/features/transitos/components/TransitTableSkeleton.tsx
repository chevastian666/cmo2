      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead className="bg-gray-900/50 border-b border-gray-700">
            <tr>
              {Array.from({ length: 9 }).map((__, i) => (
                <th key={_i} className="px-4 py-3 text-left">
          ...
          <tbody className="divide-y divide-gray-700">
            {Array.from({ length: 10 }).map((__, rowIndex) => (
              <tr key={_rowIndex} className="hover:bg-gray-700/50 transition-colors">
                {Array.from({ length: 9 }).map((__, colIndex) => (
                  <td key={_colIndex} className="px-4 py-3">
        ...
        <div className="flex gap-2">
          ...
          <div className="flex gap-1">
            {Array.from({ length: 3 }).map((__, i) => (
              <div key={_i} className="h-8 bg-gray-700 rounded animate-pulse w-8"></div>
            ))}
          </div>
          <div className="h-8 bg-gray-700 rounded animate-pulse w-20"></div>
        </div>
