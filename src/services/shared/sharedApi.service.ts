...
class SharedApiService {
  ...
  private getCacheKey(endpoint: string, params?: unknown): string {
    const paramStr = params ? JSON.stringify(_params) : ''
    return `${SHARED_CONFIG.CACHE_PREFIX}${_endpoint}_${_paramStr}`
  }
  ...
  async request<T>(
    ...
    const url = formatApiEndpoint(_endpoint)
    const cacheKey = this.getCacheKey(_endpoint, data)
    const requestOptions: RequestInit = {
      ...
      body: data && method !== 'GET' ? JSON.stringify(_data) : undefined
    }
    ...
    if (method === 'GET' && useCache) {
      ...
      const pending = this.pendingRequests.get(_cacheKey)
    ...
    if (SHARED_CONFIG.ENABLE_MOCK_DATA && !import.meta.env.VITE_USE_REAL_API) {
      const mockData = await this.getMockData<T>(_endpoint, requestOptions)
      if (mockData !== null) {
        // Cache mock data
        if (method === 'GET' && useCache) {
          this.setCache(_cacheKey, mockData)
        }
        return mockData
      }
    }
    ...
    const executeWithRetry = async (retriesLeft: number): Promise<T> => {
      try {
          ...
          if (response.status === 401 && endpoint !== '/auth/refresh') {
            ...
            if (_refreshToken) {
              try {
                ...
                const retryResponse = await fetch(_url, retryOptions)
                if (retryResponse.ok) {
        ...
        if (method === 'GET' && useCache) {
          this.setCache(_cacheKey, result)
        }
        ...
        if (retriesLeft > 0 && (_error instanceof TypeError || (_error as unknown).code === 'ECONNREFUSED')) {
          console.warn(`Request failed, retrying... (${_retriesLeft} retries left)`)
          await new Promise(resolve => setTimeout(_resolve, 1000 * (retries - retriesLeft + 1))); // Exponential backoff
    ...
    const requestPromise = executeWithRetry(_retries).finally(() => {
      this.pendingRequests.delete(_cacheKey)
    })
    // Store pending request for deduplication
    if (method === 'GET') {
      this.pendingRequests.set(_cacheKey, requestPromise)
    }
  ...
  async getCurrentUser(): Promise<Usuario | null> {
    ...
    try {
      ...
      localStorage.setItem(SHARED_CONFIG.AUTH_USER_KEY, JSON.stringify(_user))
      return user
    } catch {
      return null
    }
  ...
  async getTransito(id: string): Promise<TransitoPendiente> {
    return this.request<TransitoPendiente>('GET', `/transitos/${_id}`)
  }

  async updateTransito(id: string, data: Partial<TransitoPendiente>): Promise<TransitoPendiente> {
    const result = await this.request<TransitoPendiente>('PUT', `/transitos/${_id}`, data)
    this.clearCache('transitos')
    return result
  }

  async precintarTransito(transitoId: string, precintoData: unknown): Promise<Precinto> {
    const result = await this.request<Precinto>('POST', `/transitos/${_transitoId}/precintar`, precintoData)
    this.clearCache('transitos')
  ...
  async addObservacion(transitoId: string, observacion: string): Promise<void> {
    await this.request('POST', `/transitos/${_transitoId}/observaciones`, { observacion })
    this.clearCache(`transitos/${_transitoId}`)
  }
  ...
  async getPrecinto(id: string): Promise<Precinto> {
    return this.request<Precinto>('GET', `/precintos/${_id}`)
  }

  async updatePrecinto(id: string, data: Partial<Precinto>): Promise<Precinto> {
    const result = await this.request<Precinto>('PUT', `/precintos/${_id}`, data)
    this.clearCache('precintos')
    return result
  }

  async finalizarPrecinto(id: string, motivo: string): Promise<void> {
    await this.request('POST', `/precintos/${_id}/finalizar`, { motivo })
    this.clearCache('precintos')
  }

  // Alert endpoints
  async getAlertas(filtros?: unknown): Promise<Alerta[]> {
    const params = new URLSearchParams(_filtros).toString()
    const endpoint = params ? `/alertas?${_params}` : '/alertas'
  ...
  async atenderAlerta(id: string): Promise<void> {
    await this.request('POST', `/alertas/${_id}/atender`)
    this.clearCache('alertas')
  }

  async asignarAlerta(alertaId: string, usuarioId: string, notas?: string): Promise<void> {
    await this.request('POST', `/alertas/${_alertaId}/asignar`, { usuarioId, notas })
    this.clearCache('alertas')
  }

  async comentarAlerta(alertaId: string, mensaje: string): Promise<void> {
    await this.request('POST', `/alertas/${_alertaId}/comentarios`, { mensaje })
  }
  ...
  async resolverAlerta(
    ...
    await this.request('POST', `/alertas/${_alertaId}/resolver`, { tipo, descripcion, acciones })
    this.clearCache('alertas')
  }
  ...
  async getEstadisticasTransitos(periodo?: string): Promise<unknown> {
    const params = periodo ? `?periodo=${_periodo}` : ''
    return this.request('GET', `/estadisticas/transitos${_params}`)
  }

  async getEstadisticasAlertas(periodo?: string): Promise<unknown> {
    const params = periodo ? `?periodo=${_periodo}` : ''
    return this.request('GET', `/estadisticas/alertas${_params}`)
  }
  ...
  async buscarVehiculo(criterio: string): Promise<unknown[]> {
    return this.request('GET', `/vehiculos/buscar?q=${encodeURIComponent(_criterio)}`)
  }
  ...
  async getCMOMessages(unreadOnly = false): Promise<unknown[]> {
    ...
    return this.request('GET', `/cmo/messages${_params}`)
  }

  async markMessageAsRead(messageId: string): Promise<void> {
    await this.request('POST', `/cmo/messages/${_messageId}/read`)
  }

  async respondToMessage(messageId: string, response: string): Promise<void> {
    await this.request('POST', `/cmo/messages/${_messageId}/respond`, { response })
  }
  ...
  async exportData(
    ...
    const response = await fetch(
      formatApiEndpoint(`/export/${_type}?${_params}`),
      {
        headers: {
          ...jwtService.getAuthHeader()
        }
  ...
  private async getMockData<T>(endpoint: string, options: RequestInit): Promise<T | null> {
    // Simulate API delay
    await new Promise(resolve => setTimeout(_resolve, 300 + Math.random() * 200))
