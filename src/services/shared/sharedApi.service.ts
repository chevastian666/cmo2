...
import { generateMockAlerta, generateMockPrecinto, generateMockTransito } from '../../utils/mockData'
...
class SharedApiService {
  ...
  async request<T>(
    ...
    const executeWithRetry = async (retriesLeft: number): Promise<T> => {
      try {
        ...
        if (retriesLeft > 0 && (error instanceof TypeError || (error as ErrorWithCode).code === 'ECONNREFUSED')) {
  ...
  subscribeToUpdates(_eventType: string, _callback: (data: unknown) => void): () => void {
  ...
  // Mock data generator
  private async getMockData<T>(endpoint: string, _options: RequestInit): Promise<T | null> {
    // Simulate API delay
    await new Promise(resolve => setTimeout(resolve, 300 + Math.random() * 200))
    // Generate mock data based on endpoint
    if (endpoint.includes('/transitos/pendientes')) {

      return Array.from({ length: 10 }, (_, i) => generateMockTransito(i)) as T
    }
    
    if (endpoint.includes('/precintos/activos')) {

      return Array.from({ length: 10 }, (_, i) => generateMockPrecinto(i)) as T
    }
    
    if (endpoint.includes('/alertas/activas')) {

      return Array.from({ length: 10 }, (_, i) => generateMockAlerta(i)).filter(a => !a.atendida) as T
    }
    
    if (endpoint.includes('/alertas')) {

      return Array.from({ length: 10 }, (_, i) => generateMockAlerta(i)) as T
    }
