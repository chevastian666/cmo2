...
describe('PrecintosStore', () => {
  beforeEach(() => {
    // Reset store state
    // TODO: Implement proper store setup
    // act(() => {
  ...
  describe('fetchPrecintos', () => {
    ...
    it('handles fetch error', async () => {
      ...
      vi.mocked(precintosService.getPrecintos).mockRejectedValue(error)
      // await act(async () => {
      //   await result.current.fetchPrecintos()
      // })
      // expect(result.current.precintos).toEqual([])
    ...
    it('sets loading state correctly', async () => {
      vi.mocked(precintosService.getPrecintos).mockImplementation(() => new Promise(resolve => setTimeout(() => resolve({ data: [], pagination: { page: 1, limit: 10, total: 0, totalPages: 0 } }), 100)))
  ...
  describe('addPrecinto', () => {
    ...
    it('handles create error', async () => {
      ...
      vi.mocked(precintosService.createPrecinto).mockRejectedValue(error)
      // await act(async () => {
      //   try {
