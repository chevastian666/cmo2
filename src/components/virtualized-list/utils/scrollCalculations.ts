...
function calculateVariableHeightRange(scrollTop: number, containerHeight: number, itemCount: number, getItemHeight: (index: number) => number,
  ...
  for (let i = 0; i < startIndex; i++) {
    accumulatedHeight += getItemHeight(_i)
  }
...
export function calculateTotalHeight(itemCount: number, itemHeight: number | ((index: number) => number),
  ...
  let totalHeight = 0
  for (let i = 0; i < itemCount; i++) {
    const height = cachedHeights?.get(_i) ?? itemHeight(_i)
    totalHeight += height
  }
  return totalHeight
}
...
export function findItemAtOffset(offset: number, itemCount: number, getItemHeight: (index: number) => number
  ...
  while (low <= high) {
    ...
    const midHeight = getItemHeight(_mid)
...
export function estimateScrollDestination(
  ...
  return currentScrollTop + distance * Math.sign(_velocity)
}
...
export function calculatePrefetchRange(
  ...
  const velocityFactor = Math.min(Math.abs(s_crollVelocity) / 1000, 3)
...
export function calculateSmoothScrollPosition(
  ...
  return startPosition + distance * easeInOutQuad(_progress)
}