...
import { formatters, scales, tooltip} from './utils'
  ...
  const drawNetwork = useCallback(() => {
    ...
    links
      .on('mouseenter', function(event, d) {
        ...
        // Handle source and target properly
        ...
        const content = `
          <div class="font-semibold">${sourceId} → ${targetId}</div>
      ...
      .on('mouseleave', function() {
        d3.select(this)
          ...
          .attr('stroke-width', function(this: SVGLineElement) {
            const d = d3.select(this).datum() as SimulationLink
    ...
    const circles = nodes.append('circle')
      .attr('r', 0)
      .attr('fill', (d: NetworkNode) => colorScale(d.group) as string)
      .attr('stroke', '#1F2937')
      .attr('stroke-width', 2)
    ...
    nodes
      .on('mouseenter', function(event, d: NetworkNode) {
        ...
        links
          .style('stroke-opacity', (l: SimulationLink) => {
      ...
      .on('mouseleave', function(_event, d: NetworkNode) {
        const circle = d3.select(this).select('circle')
        const label = d3.select(this).select('text')
    ...
    buttonGroups.append('text')
      ...
      .style('fill', '#D1D5DB')
      .style('font-size', '11px')
      .text((d: { label: string; action: () => void }) => d.label)
    ...
    simulation.on('tick', () => {
      ...
      nodes.attr('transform', (d: SimulationNode) => `translate(${d.x || 0},${d.y || 0})`)
    })
